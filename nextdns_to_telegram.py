...
async def process_credentials():
    ...
    if cf_api_token and cf_account_id:
        cf_queries_str = get_cloudflare_queries(cf_account_id, cf_api_token)
        current_date = datetime.datetime.now(datetime.timezone.utc).strftime('%d-%m-%y')
        
        try:
            cf_queries_int = int(cf_queries_str)
            cf_percentage = (cf_queries_int / 100000) * 100
            cf_message = f"<pre>Total Cloudflare Workers/Pages Queries today {current_date}:\n{cf_queries_int} / 100000\n{cf_percentage:.2f}%</pre>"
        except ValueError:
            cf_message = f"<pre>Total Cloudflare Workers/Pages Queries today {current_date}:\n{cf_queries_str}</pre>"
            
        messages.insert(0, cf_message)
